...
import time
import datetime
import timeit #timeit module use to find time require to compute calculation
# Get I2C bus
...
text_file = open("/media/ubuntu/UBUNTU 16_0/oct21.txt","a") #data write in a sd card
stop = timeit.default_timer() 
time = stop-start
print "Time taken for the calculation was {} seconds".format(time)

with open("/media/ubuntu/UBUNTU 16_0/oct21.txt","a") as text_file:

    text_file.write(datetime.datetime.now().ctime()+ ' ')
    text_file.write( "%.2f" " " %fTemp)
    text_file.write( "%.2f" " "%pressure)
    text_file.write(" %.2f \n" %humidity )
    text_file.close()
